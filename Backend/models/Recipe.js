...
class Recipe {
    ...
    return {
      ...
      nutrition: nutritionResult.rows[0]
        ? {
            ...
            fats: nutritionResult.rows[0].fats ?? nutritionResult.rows[0].fat,
          }
        : null,
    };
  }
  ...
  static async findByUserId(userId, filters = {}) {
    let query = 'SELECT r.* FROM recipes r LEFT JOIN recipe_nutrition rn ON r.id = rn.recipe_id WHERE r.user_id = $1';
    const params = [userId];
    let paramCount = 1;
  ...
  static async getRecent(userId, limit = 5) {
    const result = await db.query(
      'SELECT r.*, rn.category FROM recipes r LEFT JOIN recipe_nutrition rn ON r.id = rn.recipe_id WHERE r.user_id = $1 ORDER BY r.created_at DESC LIMIT $2',
      [userId, limit]
    );
    return result.rows;
  }
