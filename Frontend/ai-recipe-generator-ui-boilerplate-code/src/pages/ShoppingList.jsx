...
const ShoppingList = () => {
    ...
    const handleAddToPantry = async () => {
        ...
        try {
            await api.post('/shopping-list/add-to-pantry', { itemIds: items.filter(item => item.is_checked).map(item => item.id) });
            const updatedItems = items.filter(item => !item.is_checked);
            setItems(updatedItems);
            organizeByCategory(updatedItems);
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <Navbar />
                <div className="text-gray-600 text-lg">Loading shopping list...</div>
            </div>
        );
    }
...
const AddItemModal = ({ onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const newItem = response.data?.data?.item || response.data?.data || null;
